/**
 * Setup database tables using Supabase SQL queries
 * This is an alternative to drizzle-kit push when direct DB connection fails
 */
...
async function setupDatabase() {
  ...
  try {
    const { data, error } = await supabase.rpc("exec_sql", { sql: SQL_SETUP });
    ...
    if (error) {
      ...
      console.log(
        "\n⚠️  Could not execute SQL directly. Please run the SQL manually in Supabase SQL Editor."
      );
      console.log("\nGo to: https://supabase.com/dashboard/project/msjemxedaxqrxahbzthr/sql/new");
